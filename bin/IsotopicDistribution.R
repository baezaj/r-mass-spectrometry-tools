IsotopicDistribution <- function(sequence, N15 = 0, IAA = TRUE, AcK = TRUE, charge = 1, 
  ...
  for(i in 1:(length(seq_vector))) {
    ...
    ## if AcK = TRUE
    if(seq_vector[i] == "K" & AcK == TRUE) x <- x + c(C = 8, H = 14, N = 2, O = 2, S = 0, P = 0)
    if(seq_vector[i] == "K" & AcK == FALSE) x <- x + c(C = 6, H = 12, N = 2, O = 1, S = 0, P = 0)
  ...
  ## run simulation
  sim <- replicate(1000, expr = simulation(elements))  
