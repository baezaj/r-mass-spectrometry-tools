FragmentPeptide <- function(sequence, fragments = "by", IAA = TRUE, N15 = FALSE, AcK = FALSE, custom = list()) {
  ...
  for(sequence_number in 1:length(sequence)) {
    ...
    residueMass <- function(residue) {
      ...
      if(residue == "K" & AcK == FALSE) mass = C*6 + H*12 + N*2 + O*1       # lysine
      if(residue == "K" & AcK == TRUE) mass = C*8 + H*14 + N*2 + O*2      # lysine, acetylated lysine
    ...
    pm <- sum(masses)
    p1 <- round(pm + H*2 + O + proton, digits = 5)               # precursor ion m/z, z = +1
    p2 <- round((pm + H*2 + O + (2 * proton)) / 2, digits = 5)   # precursor ion m/z, z = +2
    p3 <- round((pm + H*2 + O + (3 * proton)) / 3, digits = 5)   # precursor ion m/z, z = +3
    ...
    if(fragments == "by") {
      ...
      for(i in 1:(peptide_length - 1)) {                       # b-ions
        mass  <- sum(masses[1:i])
        b1[i] <- round(mass + proton, digits = 5)            # +1 charge                         
        b2[i] <- round((b1[i] + proton) / 2, digits = 5)     # +2 charge
      ...
      for(j in 2:peptide_length) {                                          # y-ions
        ...
        y1[j-1] <- round(mass + H*2 + O + proton, digits = 5)               # +1 charge
        y2[j-1] <- round((y1[j-1] + proton) / 2, digits = 5)                # +2 charge
    ...
    if(fragments == "cz") {
      ...
      for(i in 1:(peptide_length - 1)) {                        # c-ions
        mass  <- sum(masses[1:i])
        c1[i] <- round(mass + 3*H + N + proton, digits = 5)   # +1 charge                         
        c2[i] <- round((c1[i] + proton) / 2, digits = 5)      # +2 charge
      ...
      for(j in 2:peptide_length) {                                            # z-ions
        mass    <- sum(masses[j:peptide_length])                  
        z1[j-1] <- round(mass + O - N, digits = 5)                          # +1 charge
        z2[j-1] <- round((z1[j-1] + proton) / 2, digits = 5)                # +2 charge
